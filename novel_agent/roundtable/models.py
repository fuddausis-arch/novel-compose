...
- 持久化改为 SQLite（store.py），不再用 JSON 文件
- 每个会话持有 EventBroadcaster，用于 SSE 实时事件推送
"""
import asyncio
import uuid
import json
...
from novel_agent.roundtable.events import EventBroadcaster
...
class Seat:
    ...
    status: str = RoundtableStatus.IDLE.value  # RoundtableStatus: "idle" | "speaking" | "thinking" | "done"
    ...
    def from_dict(cls, data: dict) -> "Seat":
        return cls(
            ...
            status=data.get("status", RoundtableStatus.IDLE.value),
        )
...
class RoundtableSession:
    ...
    NovelAgent 适配：
    - event_broadcaster: SSE 事件广播器（替代 DeterminFlow 的全局 event_bus）
    - 持久化改为 SQLite（store.py）
    """
    ...
    def __init__(
        ...
        self.status: str = RoundtableStatus.WAITING.value  # RoundtableStatus: "waiting" | "discussing" | "paused" | "ended"
        ...
        # NovelAgent 适配：SSE 事件广播器（运行时对象，不序列化）
        self.event_broadcaster: EventBroadcaster = EventBroadcaster()
    ...
    async def emit_event(self, event: dict) -> None:
        """向所有 SSE 订阅者广播事件（替代 DeterminFlow 的 event_bus.emit_chat）。"""
        await self.event_broadcaster.emit(event)
    ...
    def pause(self) -> bool:
        """暂停会议（Phase 3）"""
        if self.status != RoundtableStatus.DISCUSSING.value:
            return False
        self.status = RoundtableStatus.PAUSED.value
    ...
    def resume(self) -> bool:
        """恢复会议（Phase 3）"""
        if self.status != RoundtableStatus.PAUSED.value:
            return False
        self.status = RoundtableStatus.DISCUSSING.value
    ...
    def from_dict(cls, data: dict) -> "RoundtableSession":
        ...
        session.status = data.get("status", RoundtableStatus.ENDED.value)
        ...
        # NovelAgent 适配：重新初始化事件广播器
        session.event_broadcaster = EventBroadcaster()
    ...
    def save(self) -> bool:
        """将会话状态持久化到 SQLite。返回 True 表示成功，False 表示失败。"""
        ok = _store.save(self.to_dict())
        if ok:
            logger.debug(f"Roundtable {self.session_id} 已保存")
        else:
            logger.error(f"保存 Roundtable {self.session_id} 失败")
        return ok
